...
import { useMyJourneys } from '../queries';
...
function JourneyEntry({
  ...
  return (
    <div className="flex items-stretch gap-3 group text-left">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl px-4 py-3 shadow-[0_1px_4px_rgba(0,0,0,0.06)] group-hover:shadow-[0_2px_8px_rgba(0,0,0,0.1)] transition-shadow">
          ...
          {hasNoRoutePoints && (
            <p className="mb-2 text-[10px] text-amber-600 bg-amber-50 rounded-lg px-2 py-1">
              Session exists, but route points were not captured.
            </p>
          )}

          <Link
            href={href}
            className="flex items-center justify-center w-full rounded-xl bg-[#0B1215] text-white text-[11px] font-bold py-2 hover:bg-[#1a2428] transition-colors"
          >
            View Journey
          </Link>
        </div>
...
interface JourneyHistoryPanelProps {
  agentName: string;
}

export function JourneyHistoryPanel({ agentName }: JourneyHistoryPanelProps) {
  ...
  return (
    <div className="flex flex-col rounded-3xl overflow-hidden shadow-[0px_4px_14px_rgba(9,35,45,0.18)]">
      {/* Header */}
      <div className="bg-[#09232D] px-5 py-5 shrink-0 text-left">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-white/5 border border-white/10 rounded-2xl px-3 py-3 flex flex-col items-center gap-0.5">
      ...
      <div className="bg-[#F8F9FA] flex-1 overflow-y-auto max-h-80 p-4 pt-5 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-200">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-2 text-center">
            ...
            <p className="text-[12px] text-gray-400 font-semibold">No journeys yet</p>
