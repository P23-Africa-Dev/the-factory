...
/**
 * Must portal to document.body. The map page is `fixed inset-0` (own stacking
 * context); BottomNavBar and react-modal-sheet sit outside that context, so an
 * in-tree z-index can never paint above them.
 */
export function CompleteRequirementsSheet({
  ...
  const { hide: hideBottomNav, show: showBottomNav } = useBottomNavVisibility();
  ...
  }, [visible, hideBottomNav, showBottomNav]);
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[70000] flex items-end sm:items-center justify-center bg-[#051014]/70 backdrop-blur-sm p-0 sm:p-5 font-sans"
      role="dialog"
      aria-modal="true"
      aria-labelledby="complete-task-title"
    >
      <div
        className="relative bg-[#0B3343] border border-white/10 rounded-t-2xl sm:rounded-2xl w-full max-w-md shadow-2xl flex flex-col gap-4 text-white overflow-hidden"
        style={{
          maxHeight: 'min(92vh, calc(100dvh - env(safe-area-inset-top, 0px) - env(safe-area-inset-bottom, 0px)))',
        }}
      >
        <div className="overflow-y-auto overscroll-contain p-6 flex flex-col gap-4 pb-[max(1.5rem,env(safe-area-inset-bottom,0px))]">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 id="complete-task-title" className="font-bold text-xl text-white">
                Complete task
              </h3>
            ...
            {onDismiss && (
              <button
                type="button"
                onClick={onDismiss}
                className="h-9 w-9 rounded-full bg-white/10 flex items-center justify-center text-white/70 hover:text-white shrink-0"
                aria-label="Close"
              >
                <X size={16} />
              </button>
