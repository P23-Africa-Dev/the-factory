...
function AgentDetailSidebar({ agent, companyId }: { agent: Agent; companyId?: number | string }) {
  ...
  const [phone, setPhone] = useState(agent.phone === '—' ? '' : agent.phone);
  const [role, setRole] = useState<string>(agent.internalRole);

  const user = useAuthStore((s) => s.user);
  const { apiCompanyId: activeCompanyId } = getActiveCompanyContext(user);
  const effectiveCompanyId = companyId ?? activeCompanyId ?? undefined;
  const updateMutation = useUpdateInternalUser();
  const { data: zones = [] } = useCompanyZones(effectiveCompanyId);
  ...
  const handleEdit = () => {
    ...
    setPhone(agent.phone === '—' ? '' : agent.phone);
    setRole(agent.internalRole);
    setEditing(true);
  };
  ...
  return (
    <div className="flex flex-col gap-5 w-full xl:w-90 xl:shrink-0">
      ...
      <div>
        ...
        {/* Lifecycle + quick actions: Suspend on the left, quick icons on the right */}
