  ...
  return (
    <nav className="h-20 flex items-center justify-between px-6 lg:px-10 bg-[#09232D] text-white sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4 lg:gap-8">
        ...
        <div ref={profileRef} className="relative flex items-center gap-3 lg:gap-4">
          <div className="hidden sm:block text-right">
            <p className="text-sm font-bold tracking-tight">{user?.name ?? "—"}</p>
            <p className="text-[11px] text-white/30 font-medium tracking-wide">{user?.email ?? "—"}</p>
          </div>
              ...
              >
                ...
                <div className="px-4 py-3.5 border-b border-white/10 flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-white/10 border border-white/10 flex items-center justify-center shrink-0">
                    {user?.avatar ? (
                      <Image src={user.avatar} alt="Profile" width={36} height={36} className="w-full h-full object-cover rounded-full" />
                    ) : (
