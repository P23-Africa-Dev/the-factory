...
export function PayrollList({ selectedId, onSelect, showViewAll = false }: PayrollListProps) {
  const displayedAgents = showViewAll ? agents : agents.slice(0, 3);

  return (
    <div className="bg-white rounded-[24px] p-5 sm:p-6 shadow-sm flex-1 min-w-0">
      {/* Header */}
      {!showViewAll && (
        ...
        {displayedAgents.map((agent) => {
          ...
          return (
            ...
            >
              {/* Left accent bar */}
              <div
                className={`w-[6px] self-stretch shrink-0 rounded-l-[20px] ${
              ...
              <div className="min-w-0 flex-1 sm:flex-none sm:w-40 lg:w-44 py-3">
                ...
                <p className={`text-[9px] sm:text-[10px] mt-0.5 truncate ${isSelected ? 'text-white/40' : 'text-gray-400'}`}>
                  {agent.address}
                </p>
              </div>
              ...
              <div className="hidden sm:block flex-1 min-w-0 py-3">
                <p className={`text-[13px] font-semibold ${isSelected ? 'text-white' : 'text-[#0B1215]'}`}>
                  {agent.baseSalary}
                </p>
              </div>
