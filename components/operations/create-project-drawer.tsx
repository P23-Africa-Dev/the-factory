...
const CATEGORY_OPTIONS  = ['Outreach', 'Sales', 'Inspection', 'Delivery', 'Awareness', 'Other'];
const LEAD_OPTIONS      = ['Tunde Balogun', 'Ridwan Thomson', 'Amaka Osei', 'Abdul Kareem'];
...
const FIELD = 'w-full bg-[#F6F6F6] border border-gray-200 rounded-2xl px-4 py-3.5 text-[13px] text-[#09232D] outline-none focus:ring-2 focus:ring-[#09232D]/10 focus:border-[#09232D]/30 transition-all placeholder:text-gray-300';
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    ...
    >
      ...
      {checked && (
        <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[9px] font-black text-white">I</span>
      )}
    </div>
  );
}
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-full max-w-[440px] bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        ...
        <div className="relative px-7 pt-8 pb-6 shrink-0 overflow-hidden">
          ...
          <div className="relative z-10 flex items-start justify-between">
            <div>
              <h2 className="text-[20px] font-bold text-[#09232D]">Create New Project</h2>
        ...
        >
          ...
          <Row label="Project Lead">
            <div className="relative">
              <User size={13} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
              <select
                value={form.lead}
                onChange={(e) => set('lead', e.target.value)}
                className={`${FIELD} appearance-none pl-9 pr-9 cursor-pointer`}
              >
                <option value="" disabled>Assign a lead</option>
