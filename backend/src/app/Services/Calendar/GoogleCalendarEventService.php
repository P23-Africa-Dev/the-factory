...
class GoogleCalendarEventService
{
    ...
    public function upsertMeeting(Meeting $meeting, CompanyCalendarConnection $connection): array
    {
        ...
        if ($eventId !== '') {
            $response = Http::withToken($accessToken)
                ->timeout(30)
                ->patch(
                    'https://www.googleapis.com/calendar/v3/calendars/primary/events/' . urlencode($eventId) . '?conferenceDataVersion=1&sendUpdates=all',
                    $payload,
                );
        } else {
        ...
        if (! $response->successful()) {
            throw ValidationException::withMessages([
                'integration' => ['Google Calendar event sync failed.'],
            ]);
        }
        ...
        return [
            ...
            'calendar_id' => Arr::get($event, 'organizer.email'),
    ...
    public function cancelMeeting(Meeting $meeting, CompanyCalendarConnection $connection): void
    {
        ...
        if (! $response->successful()) {
            throw ValidationException::withMessages([
                'integration' => ['Google Calendar event cancel failed.'],
            ]);
        }
    }
        ...
        return [
            ...
            'attendees' => $meeting->attendees
                ->map(static fn($attendee): array => [
                    ...
                    'optional' => (bool) $attendee->is_optional,
                    'responseStatus' => (string) $attendee->response_status,
