...
class CalendarIntegrationTest extends TestCase
{
    ...
    public function test_non_owner_cannot_request_google_calendar_connect_url(): void
    {
        ...
        $response->assertUnprocessable()
            ...
            ->assertJsonPath(
                ...
                'Only company owners can connect or disconnect Google Calendar integration.',
            );
    }

    public function test_management_user_can_view_calendar_integration_status(): void
    {
    ...
    public function test_owner_can_disconnect_calendar_integration(): void
    {
        [$company, $owner] = $this->seedCompanyUsers();
    ...
    public function test_oauth_callback_rejects_when_user_is_no_longer_owner(): void
    {
        [$company, $owner] = $this->seedCompanyUsers();
        ...
        $this->getJson('/api/v1/calendar/integration/callback?code=oauth-code&state=' . urlencode($state))
            ->assertUnprocessable()
            ->assertJsonPath('success', false)
            ->assertJsonPath('errors.integration.0', 'Only the current company owner can complete Google Calendar connection.');
    }
