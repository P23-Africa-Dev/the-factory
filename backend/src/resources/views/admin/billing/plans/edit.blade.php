...
@section('page-title', 'Edit Billing Plan')
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">{{ $plan->label }}</li>
@endsection

@section('content')
    <div class="metric-card p-4">
        <form method="POST" action="{{ route('admin.billing.plans.update', $plan) }}" class="row g-3">
            @csrf
            @method('PATCH')

            <div class="col-md-4">
                <label class="form-label small fw-semibold">Plan Key</label>
                <input type="text" name="plan_key" class="form-control" value="{{ old('plan_key', $plan->plan_key) }}"
                    required>
            </div>

            <div class="col-md-8">
                <label class="form-label small fw-semibold">Label</label>
                <input type="text" name="label" class="form-control" value="{{ old('label', $plan->label) }}" required>
            </div>

            <div class="col-md-4">
                <label class="form-label small fw-semibold">Seat Limit</label>
                <input type="number" min="1" name="seat_limit" class="form-control"
                    value="{{ old('seat_limit', $plan->seat_limit) }}" required>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="number" min="0" name="monthly_amount" class="form-control"
                    value="{{ old('monthly_amount', $plan->monthly_amount) }}" required>
            </div>
            ...
            <div class="col-md-4">
                ...
                <input type="number" min="0" name="annual_amount" class="form-control"
                    value="{{ old('annual_amount', $plan->annual_amount) }}" required>
            </div>

            <div class="col-md-6">
                <label class="form-label small fw-semibold">Monthly Stripe Price ID</label>
                <input type="text" name="monthly_price_id" class="form-control"
                    value="{{ old('monthly_price_id', $plan->monthly_price_id) }}" placeholder="price_...">
            </div>

            <div class="col-md-6">
                <label class="form-label small fw-semibold">Annual Stripe Price ID</label>
                <input type="text" name="annual_price_id" class="form-control"
                    value="{{ old('annual_price_id', $plan->annual_price_id) }}" placeholder="price_...">
            </div>
