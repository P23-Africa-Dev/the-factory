...
<div class="row g-3">
    ...
    <div class="col-lg-8">
        ...
        <div class="metric-card p-4 mb-3">
            ...
            @if ($isEnterprise && $company)
                ...
                <div class="detail-row">
                    ...
                    <div class="detail-value">
                        <code style="font-size:.82rem;background:var(--bg-secondary,#f1f5f9);padding:.2em .4em;border-radius:.25rem">{{ $company->company_id }}</code>
                    </div>
                </div>
            ...
            @if ($isEnterprise && $user->enterprise_onboarding_completed_at)
                <div class="detail-row">
                    ...
                    <div class="detail-value">{{ $user->enterprise_onboarding_completed_at->format('M j, Y H:i') }}</div>
                </div>
        ...
        <div class="metric-card p-4 mb-3">
            ...
            <div class="detail-row">
                ...
                <div class="detail-value">
                    ...
                    @endphp
                    <span class="badge-status" style="background:{{ $roleColor['bg'] }};color:{{ $roleColor['fg'] }}">
            ...
            @if ($accountOwner)
                <div class="detail-row">
                    ...
                    <div class="detail-value">
                        <a href="{{ route('admin.users.show', $accountOwner) }}"
                           class="text-decoration-none" style="color:var(--accent)">
                            {{ $accountOwner->name }}
                        </a>
            ...
            @foreach ($user->agents as $agent)
                <div class="detail-row">
                    <div class="detail-label" style="flex:0 0 auto">
                        <span class="badge-status" style="background:rgba(6,182,212,.12);color:#0891b2;font-size:.68rem">Agent</span>
                    </div>
                    <div class="detail-value">
                        <a href="{{ route('admin.users.show', $agent) }}"
                           class="text-decoration-none fw-semibold" style="font-size:.85rem;color:var(--text-primary)">
                            {{ $agent->name }}
                        </a>
        ...
        <div class="metric-card p-4">
            ...
            @foreach ([
                ['Registered',      $user->created_at?->format('M j, Y H:i')],
                ['Email Verified',  $user->email_verified_at?->format('M j, Y H:i') ?? 'Not verified'],
                ['Deactivated',     $user->deactivated_at?->format('M j, Y H:i') ?? '—'],
                ['Suspended Until', $user->suspended_until?->format('M j, Y H:i') ?? '—'],
            ] as [$label, $value])
                <div class="detail-row">
                    <div class="detail-label">{{ $label }}</div>
                    <div class="detail-value">{{ $value }}</div>
        ...
        <div class="metric-card p-4 mb-3">
            ...
            @if ($user->is_active && !$suspended)
                <div class="d-grid gap-2">
                    <button class="btn btn-outline-warning btn-sm" data-bs-toggle="modal" data-bs-target="#suspendModal">
            ...
            @else
                <form method="POST" action="{{ route('admin.users.reactivate', $user) }}" class="d-grid">
                    @csrf
                    <button class="btn btn-sm" style="background:rgba(16,185,129,.1);color:#059669;border:1px solid rgba(16,185,129,.2)">
                ...
                <div class="mb-2">
                    <select name="internal_role" class="form-select form-select-sm"
                        ...
                        @foreach (['agent', 'supervisor', 'manager'] as $role)
                            <option value="{{ $role }}" @selected($user->internal_role === $role)>{{ ucfirst($role) }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="d-grid">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('admin.users.suspend', $user) }}">
                ...
                <div class="modal-body">
                    <p style="font-size:.85rem;color:var(--text-secondary)" class="mb-3">Choose suspension duration:</p>
