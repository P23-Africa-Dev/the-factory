        ...
        <div class="metric-card p-4 mb-4">
            ...
            @if ($canManageAiStack)
                <form method="POST" action="{{ route('admin.ai.stack.update') }}" class="row g-3 align-items-stretch">
                    @csrf
                    <div class="col-md-4">
                        <input type="radio" class="btn-check" name="stack" id="ai-stack-openai-claude"
                            value="openai_claude" {{ $activeAiStack === 'openai_claude' ? 'checked' : '' }}>
                ...
                <div class="mt-2 alert alert-warning mb-0 py-2 px-3" style="font-size:.78rem">
                    Hosted NVIDIA NIM (API catalog) can be slow or hang under load, that is NVIDIA-side contention, not a
                    Factory23 outage.
            ...
            <div class="d-flex align-items-center gap-4 flex-wrap ai-status-panel-metrics">
                <div style="font-size:.82rem">
                    <span class="me-3">OpenAI:
                        <strong
                            id="realtime-status-openai">{{ $openaiHealth['label'] ?? ($openaiHealth['ok'] ?? false ? 'Connected' : 'Unavailable') }}</strong></span>
                    <span class="me-3">Claude:
                        <strong
                            id="realtime-status-claude">{{ $claudeHealth['label'] ?? ($claudeHealth['ok'] ?? false ? 'Connected' : 'Unavailable') }}</strong></span>
                    <span>NVIDIA:
                        <strong
                            id="realtime-status-nvidia">{{ $nvidiaHealth['label'] ?? ($nvidiaHealth['ok'] ?? false ? 'Connected' : 'Unavailable') }}</strong></span>
                    <span class="ms-3">GLM:
                        <strong
                            id="realtime-status-glm">{{ $glmHealth['label'] ?? ($glmHealth['ok'] ?? false ? 'Connected' : 'Unavailable') }}</strong></span>
                </div>
        ...
        <div class="row g-3 mb-4">
            ...
            'glm' => ['label' => 'GLM (Zhipu)', 'health' => $glmHealth],
                ...
                @php
                    ...
                    $isActiveStackVendor =
                        ($activeAiStack === 'nvidia' && $key === 'nvidia') ||
                        ($activeAiStack === 'glm' && $key === 'glm') ||
                        ($activeAiStack === 'openai_claude' && in_array($key, ['openai', 'claude'], true));
                @endphp
                <div class="col-lg-3">
                    <div class="provider-health-card {{ $presentation['card_class'] }}"
                        ...
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            ...
                            <span class="health-pill {{ $presentation['pill_class'] }}"
                                id="provider-pill-{{ $key }}">{{ $presentation['label'] }}</span>
                        </div>
                        <div class="row g-2 mb-3" style="font-size:.78rem">
                            ...
                            <div class="col-6">
                                ...
                                <div class="fw-600">
                                    {{ isset($health['latency_ms']) ? $health['latency_ms'] . 'ms' : '—' }}
                                </div>
                            </div>
                            <div class="col-6">
                                <div style="color:var(--text-muted)">Status Message</div>
                                <div id="provider-message-{{ $key }}">
                                    {{ $presentation['message'] !== '' ? $presentation['message'] : '—' }}</div>
                            </div>
                        </div>
                        <div
                            style="background:var(--surface-hover);border-radius:.5rem;padding:.75rem;margin-bottom:.75rem;font-size:.78rem">
                            <div class="row g-2">
                                <div class="col-4"><span style="color:var(--text-muted)">LLM
                                        Today</span><br><strong>{{ number_format($usage['requests_today'] ?? 0) }}</strong>
                                </div>
                                <div class="col-4"><span style="color:var(--text-muted)">LLM
                                        Month</span><br><strong>{{ number_format($usage['requests_month'] ?? 0) }}</strong>
                                </div>
                                <div class="col-4"><span style="color:var(--text-muted)">Est.
                                        Cost</span><br><strong>${{ number_format($usage['estimated_cost_usd'] ?? 0, 4) }}</strong>
                                </div>
                                <div class="col-4"><span style="color:var(--text-muted)">Input
                                        Tokens</span><br><strong>{{ number_format($usage['input_tokens'] ?? 0) }}</strong>
                                </div>
                                <div class="col-4"><span style="color:var(--text-muted)">Output
                                        Tokens</span><br><strong>{{ number_format($usage['output_tokens'] ?? 0) }}</strong>
                                </div>
                                <div class="col-4"><span style="color:var(--text-muted)">Total
                                        Tokens</span><br><strong>{{ number_format($usage['total_tokens'] ?? 0) }}</strong>
                                </div>
                            </div>
                        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <div class="fw-bold text-capitalize">
                        {{ in_array($activeAiStack, ['nvidia', 'glm'], true) ? $activeAiStack : $primaryProvider }}</div>
                </div>
                <div class="col-md-3">
                    <div style="font-size:.75rem;color:var(--text-muted)">Fallback Provider</div>
                    <div class="fw-bold text-capitalize">
                        {{ in_array($activeAiStack, ['nvidia', 'glm'], true) ? 'none (hard switch)' : $fallbackProvider }}
                    </div>
                </div>
                <div class="col-md-3">
        ...
        @if ($aiLogsReady)
                    ...
                    <div class="metric-card p-4 h-100">
                        ...
                        @forelse($topUsers as $user)
                            <div class="d-flex justify-content-between py-2"
                                style="border-bottom:1px solid var(--border-light);font-size:.82rem">
                                <div>
                        ...
                        @forelse($modelUsageDetailed as $model)
                            <div class="mb-2">
                                ...
                                <div style="height:5px;background:var(--border-light);border-radius:3px;">
                                    <div
                                        style="height:100%;background:var(--accent);border-radius:3px;width:{{ $model['percentage'] }}%">
                                    </div>
                                </div>
                                <div class="d-flex justify-content-between mt-1"
                                    style="font-size:.7rem;color:var(--text-muted)">
                                    <span>{{ number_format($model['requests']) }} req</span>
                                    <span>{{ number_format($model['tokens']) }} tok ·
                                        ${{ number_format($model['cost'], 4) }}</span>
                                </div>
                            </div>
                        @empty
            ...
            <div class="metric-card mb-4">
                <div class="table-card-header">
                    ...
                    <a href="{{ route('admin.ai.logs.index', ['status' => 'failed']) }}"
                        class="btn btn-sm btn-outline-secondary">View All Errors</a>
                </div>
                            ...
                            @forelse($recentErrors as $error)
                                <tr>
                                    <td style="font-size:.78rem;white-space:nowrap">
                                        {{ isset($error['occurred_at']) ? \Illuminate\Support\Carbon::parse($error['occurred_at'])->format('M j, H:i:s') : '—' }}
                                    </td>
                                    <td style="font-size:.78rem;max-width:280px">
                                        {{ \Illuminate\Support\Str::limit($error['message'], 80) }}</td>
                                    <td style="font-size:.78rem" class="text-capitalize">{{ $error['provider'] }}</td>
                                    <td style="font-size:.78rem">{{ $error['user'] ?? '—' }}</td>
                                    <td><span class="badge-status badge-rejected">{{ ucfirst($error['status']) }}</span>
                                    </td>
                                    <td style="font-size:.78rem">{{ ucfirst($error['resolution']) }}</td>
                                    <td><a href="{{ $error['log_url'] }}" class="btn btn-sm btn-outline-secondary"><i
                                                class="bi bi-eye"></i></a></td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4" style="color:var(--text-muted)">No recent
                                        errors.</td>
                                </tr>
                            @endforelse
                        </tbody>
                        ...
                        <tbody>
                            @forelse($recentLogs as $log)
                                <tr>
                                    ...
                                    <td><span
                                            class="badge-status {{ $log->status === 'success' ? 'badge-active' : 'badge-rejected' }}">{{ ucfirst($log->status) }}</span>
                                    </td>
                                    <td style="font-size:.78rem">
                                        {{ $log->execution_ms ? $log->execution_ms . 'ms' : '—' }}</td>
                                    <td style="font-size:.78rem">
                                        {{ $log->total_tokens ? number_format($log->total_tokens) : '—' }}</td>
                                    <td style="font-size:.78rem">
                                        ${{ number_format((float) ($log->estimated_cost_usd ?? 0), 4) }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="11" class="text-center py-4" style="color:var(--text-muted)">No recent
                                        activity.</td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <script>
        ...
        function syncProviderWarningBanner(health) {
            ...
            if (!banner) {
                ...
                banner.innerHTML =
                    `<i class="bi bi-exclamation-triangle-fill me-2"></i><span class="ai-provider-warning-text"></span>`;
        ...
        function updateAggregateStatus() {
            ...
            const map = {
                online: {
                    class: 'ai-status-online',
                    icon: 'bi-check-circle-fill',
                    label: 'Online'
                },
                offline: {
                    class: 'ai-status-offline',
                    icon: 'bi-x-circle-fill',
                    label: 'Offline'
                },
                degraded: {
                    class: 'ai-status-degraded',
                    icon: 'bi-exclamation-triangle-fill',
                    label: 'Degraded'
                },
                fallback: {
                    class: 'ai-status-fallback',
        ...
        document.querySelectorAll('.test-provider-btn').forEach((btn) => {
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    box.className = 'test-result-box alert ' + (data.ok ? 'alert-success' :
                        'alert-danger');
                    box.innerHTML =
                        `<strong>${data.label || 'Result'}</strong>: ${data.message || ''}` +
                        (data.latency_ms ? ` · ${data.latency_ms}ms` : '') +
                        (data.sample_models ?
                            `<br><small>Models: ${data.sample_models.join(', ')}</small>` : '');
